...
window.J = {
  ...
  os:           [], clientes:     [], veiculos:     [],
  estoque:      [], financeiro:   [], equipe:       [],
  fornecedores: [], agendamentos: [], mensagens:    [],
  chatEquipe:   [], auditoria:    [],

  chatAtivo:    null, chatAtivoEquipe: null,
  notifLastSeen: Date.now(), db: null
};
...
window.pode = acao => { const fn = PERM[acao]; return fn ? fn(J.role) : false; };
...
window.initCore = function() {
  ...
  _populateBaseUI(); _aplicarRestricoesPorRole();
  _escutarOS(); _escutarClientes(); _escutarVeiculos(); _escutarEstoque();
  _escutarFinanceiro(); _escutarEquipe(); _escutarFornecedores();
  _escutarMensagens(); _escutarChatEquipe(); _escutarAgendamentos();
  _escutarAuditoria(); _escutarNotificacoes();
...
window.initCoreEquipe = function() {
  ...
  if (!J.tid) { window.location.replace('index.html'); return; }
  if (J.brand) window.aplicarBrand(J.brand);
  _populateBaseUI(); _escutarOS(); _escutarClientes(); _escutarVeiculos();
  _escutarEstoque(); _escutarChatEquipe(); _escutarNotificacoes();
...
function _populateBaseUI() {
  ...
  set('sbTenantNome', J.tnome); set('sbUserNome', J.nome); set('sbUserRole', _roleLabel(J.role));
...
function _roleLabel(role) { return { admin:'ADMINISTRADOR', gestor:'GESTOR MASTER', atendente:'ATENDENTE', mecanico:'MECÂNICO', equipe:'EQUIPE', cliente:'CLIENTE' }[role] || (role||'').toUpperCase(); }
...
// ── LISTENERS FIRESTORE ────────────────────────────────────
function _escutarOS() { J.db.collection('ordens_servico').where('tenantId', '==', J.tid).onSnapshot(snap => { J.os = snap.docs.map(d => ({ id: d.id, ...d.data() })); window.renderKanban && renderKanban(); window.renderDashboard && renderDashboard(); window.calcComissoes && calcComissoes(); window.atualizarPainelAtencao && atualizarPainelAtencao(); }); }
function _escutarClientes() { J.db.collection('clientes').where('tenantId', '==', J.tid).onSnapshot(snap => { J.clientes = snap.docs.map(d => ({ id: d.id, ...d.data() })); window.renderClientes && renderClientes(); popularSelects(); }); }
function _escutarVeiculos() { J.db.collection('veiculos').where('tenantId', '==', J.tid).onSnapshot(snap => { J.veiculos = snap.docs.map(d => ({ id: d.id, ...d.data() })); window.renderVeiculos && renderVeiculos(); popularSelects(); }); }
function _escutarEstoque() { J.db.collection('estoqueItems').where('tenantId', '==', J.tid).onSnapshot(snap => { J.estoque = snap.docs.map(d => ({ id: d.id, ...d.data() })); window.renderEstoque && renderEstoque(); window.renderDashboard && renderDashboard(); }); }
function _escutarFinanceiro() { J.db.collection('financeiro').where('tenantId', '==', J.tid).onSnapshot(snap => { J.financeiro = snap.docs.map(d => ({ id: d.id, ...d.data() })); window.renderFinanceiro && renderFinanceiro(); window.renderDashboard && renderDashboard(); }); }
function _escutarEquipe() { J.db.collection('funcionarios').where('tenantId', '==', J.tid).onSnapshot(snap => { J.equipe = snap.docs.map(d => ({ id: d.id, ...d.data() })); window.renderEquipe && renderEquipe(); window.calcComissoes && calcComissoes(); popularSelects(); }); }
function _escutarFornecedores() { J.db.collection('fornecedores').where('tenantId', '==', J.tid).onSnapshot(snap => { J.fornecedores = snap.docs.map(d => ({ id: d.id, ...d.data() })); window.renderFornecedores && renderFornecedores(); popularSelects(); }); }
function _escutarMensagens() { J.db.collection('mensagens').where('tenantId', '==', J.tid).onSnapshot(snap => { J.mensagens = snap.docs.map(d => ({ id: d.id, ...d.data() })).sort((a,b)=>(a.ts||0)-(b.ts||0)); window.renderChatLista && renderChatLista(); if (J.chatAtivo && window.renderChatMsgs) renderChatMsgs(J.chatAtivo); const unread = J.mensagens.filter(m => m.sender === 'cliente' && !m.lidaAdmin).length; setBadge('chatBadge', unread); }); }
function _escutarAgendamentos() { J.db.collection('agendamentos').where('tenantId', '==', J.tid).onSnapshot(snap => { J.agendamentos = snap.docs.map(d => ({ id: d.id, ...d.data() })); window.renderAgenda && renderAgenda(); }); }
function _escutarAuditoria() { J.db.collection('lixeira_auditoria').where('tenantId', '==', J.tid).onSnapshot(snap => { J.auditoria = snap.docs.map(d => ({ id: d.id, ...d.data() })).sort((a,b)=>b.ts>a.ts?1:-1); window.renderAuditoria && renderAuditoria(); }); }
function _escutarNotificacoes() { J.db.collection('notificacoes_live').where('tenantId', '==', J.tid).where('ts', '>', J.notifLastSeen).onSnapshot(snap => { snap.docChanges().forEach(change => { if (change.type !== 'added') return; const n = change.doc.data(); if (n.de === J.nome) return; window.toast && toast(`${n.de}: ${n.msg}`, 'info'); setTimeout(() => change.doc.ref.delete().catch(()=>{}), 10000); }); }); }
function _escutarChatEquipe() { J.db.collection('chat_equipe').where('tenantId', '==', J.tid).onSnapshot(snap => { J.chatEquipe = snap.docs.map(d => ({ id: d.id, ...d.data() })).sort((a,b)=>(a.ts||0)-(b.ts||0)); window.renderChatEquipe && renderChatEquipe(); window.renderChatEquipeAdmin && renderChatEquipeAdmin(); if (J.chatAtivoEquipe && window.renderChatMsgsEquipeAdmin) renderChatMsgsEquipeAdmin(J.chatAtivoEquipe); const nEquipe = J.chatEquipe.filter(m => m.sender==='admin' && !m.lidaEquipe && m.para===J.fid).length; setBadge('chatTabBadge', nEquipe); const nAdmin = J.chatEquipe.filter(m => m.sender==='equipe' && !m.lidaAdmin).length; setBadge('chatEquipeBadge', nAdmin); }); }
function _escutarComissoesEquipe() { J.db.collection('financeiro').where('tenantId', '==', J.tid).where('isComissao', '==', true).where('mecId', '==', J.fid).onSnapshot(snap => { const fins = snap.docs.map(d => ({ id: d.id, ...d.data() })); window.renderComissoes && renderComissoes(fins); }); }
...
window.atualizarPainelAtencao = function() {
  ...
  container.innerHTML = Object.entries(alertas).map(([key, cfg]) => {
    ...
    return `<div class="atencao-box" style="border-color:rgba(${_hexToRGB(cfg.cor)},0.3);background:rgba(${_hexToRGB(cfg.cor)},0.06)"><div class="atencao-titulo" style="color:${cfg.cor}">${cfg.label}</div><div class="atencao-lista">${items}</div></div>`;
  }).join('');
  const led = document.getElementById('alertaLed');
  if (led) led.style.display = temAlerta ? 'block' : 'none';
};
window._abrirDetalheOS = function(osId) { window._osDetalheAberta = osId; if (window.prepOS) { prepOS('edit', osId); abrirModal('modalOS'); } };
window.notificarEquipe = async function(msg) { try { await J.db.collection('notificacoes_live').add({ tenantId: J.tid, de: J.nome, msg, ts: Date.now() }); } catch(e) {} };

// ── POPULAR SELECTS ────────────────────────────────────────
window.popularSelects = function() {
  const cOpts = '<option value="">Selecione...</option>' + J.clientes.map(c => `<option value="${c.id}">${c.nome}</option>`).join('');
  ['osCliente','agdCliente','veicDono'].forEach(id => { const el = document.getElementById(id); if (el) el.innerHTML = cOpts; });
  const mOpts = '<option value="">Não atribuído</option>' + J.equipe.map(f => `<option value="${f.id}">${f.nome} (${JARVIS_CONST.CARGOS[f.cargo]||f.cargo})</option>`).join('');
  ['osMec','agdMec'].forEach(id => { const el = document.getElementById(id); if (el) el.innerHTML = mOpts; });
  const fOpts = '<option value="">Selecione...</option>' + J.fornecedores.map(f => `<option value="${f.id}">${f.nome}</option>`).join('');
  const nfEl = document.getElementById('nfFornec'); if (nfEl) nfEl.innerHTML = fOpts;
  const optF = document.getElementById('optFornec'); if (optF) optF.innerHTML = J.fornecedores.map(f=>`<option value="F_${f.id}">${f.nome}</option>`).join('');
  const optE = document.getElementById('optEquipe'); if (optE) optE.innerHTML = J.equipe.map(f=>`<option value="E_${f.id}">${f.nome}</option>`).join('');
  if (window.renderChatEquipeAdmin) renderChatEquipeAdmin();
};
window.filtrarVeiculosOS = function() {
  const cid  = _v('osCliente'); const tipo = _v('osTipoVeiculo'); let veics  = J.veiculos.filter(v => v.clienteId === cid);
  if (tipo) veics = veics.filter(v => v.tipo === tipo);
  const el = document.getElementById('osVeiculo');
  if (el) el.innerHTML = '<option value="">Selecione...</option>' + veics.map(v => `<option value="${v.id}">${v.modelo} (${v.placa})</option>`).join('');
};
window.filtrarVeicsAgenda = function() {
  const cid   = _v('agdCliente'); const veics = J.veiculos.filter(v => v.clienteId === cid);
...
window.buscaGlobal = function(termo) {
  const t = (termo || '').trim().toUpperCase(); const box = document.getElementById('buscaGlobalResultados');
  if (!box) return; if (!t) { box.classList.add('hidden'); return; }
  const matches = J.os.filter(o => {
      const v = J.veiculos.find(x => x.id === o.veiculoId);
      const c = J.clientes.find(x => x.id === o.clienteId);
      return (v?.placa||'').toUpperCase().includes(t) || (o.placa||'').toUpperCase().includes(t) || (c?.nome||'').toUpperCase().includes(t);
    }).sort((a,b) => (b.updatedAt||'') > (a.updatedAt||'') ? 1 : -1).slice(0, 8);
  if (!matches.length) { box.innerHTML = `<div style="padding:12px;color:var(--text-muted);font-size:0.8rem;text-align:center">Nenhum resultado</div>`; box.classList.remove('hidden'); return; }
  box.innerHTML = matches.map(o => {
    const v = J.veiculos.find(x => x.id === o.veiculoId);
    const c = J.clientes.find(x => x.id === o.clienteId);
    const STATUS_COLORS = { Triagem:'badge-neutral', Orcamento:'badge-warn', Orcamento_Enviado:'badge-purple', Aprovado:'badge-brand', Andamento:'badge-warn', Pronto:'badge-success', Entregue:'badge-success', Cancelado:'badge-danger' };
    return `<div class="busca-item" onclick="_abrirDetalheOS('${o.id}')"><div style="font-weight:700;font-family:var(--fm);letter-spacing:0.08em">${o.placa||v?.placa||'?'}</div><div style="font-size:0.78rem;color:var(--text-secondary)">${c?.nome||'—'} · ${v?.modelo||o.veiculo||''}</div><span class="badge ${STATUS_COLORS[o.status]||'badge-neutral'}" style="font-size:0.55rem;margin-top:4px;">${o.status||'?'}</span></div>`;
  }).join('');
  box.classList.remove('hidden');
};
document.addEventListener('click', e => { if (!document.getElementById('buscaGlobalWrap')?.contains(e.target)) { const box = document.getElementById('buscaGlobalResultados'); if (box) box.classList.add('hidden'); } });

// ── AUDITORIA E UTILITÁRIOS ────────────────────────────────────────────
window.audit = async function(modulo, acao) { try { await J.db.collection('lixeira_auditoria').add({ tenantId: J.tid, modulo, acao, usuario: J.nome, role: J.role, ts: new Date().toISOString() }); } catch(e) {} };
...
window.abrirWpp = function(numero, msg) { const n = (numero||'').replace(/\D/g,''); window.open(`https://wa.me/55${n}?text=${encodeURIComponent(msg||'')}`, '_blank'); };
window.setBadge = function(id, count) { const el = document.getElementById(id); if (!el) return; el.textContent = count; el.classList.toggle('show', count > 0); el.style.display = count > 0 ? 'block' : 'none'; };
window.buscarCEP = async function(cep) { const c = (cep||'').replace(/\D/g,''); if (c.length !== 8) return; try { const r = await fetch(`https://viacep.com.br/ws/${c}/json/`); const d = await r.json(); if (!d.erro) { _sv('cliRua', d.logradouro); _sv('cliBairro', d.bairro); _sv('cliCidade', d.localidade); document.getElementById('cliNum')?.focus(); } } catch(e) {} };
function _hexToRGB(hex) { const c = (hex||'#3B82F6').replace('#',''); return `${parseInt(c.substring(0,2),16)},${parseInt(c.substring(2,4),16)},${parseInt(c.substring(4,6),16)}`; }

// ============================================================
// FORMATADOR BLINDADO DE MÍDIA (CORRIGIDO ".includes")
// ============================================================
...
// ============================================================
// MOTOR DE MÍDIA DO CHAT (ÁUDIO E ARQUIVOS) - CLICK TO RECORD
// ============================================================
let _mediaRecorder;
let _audioChunks = [];
let _recordingType = null;

window.togglePTT = async function(tipo = 'CRM') {
  const btnId = tipo === 'Equipe' ? 'btnPTT_Equipe' : 'btnPTT_CRM';
  ...
  try {
    ...
    _mediaRecorder.onstop = async () => {
      ...
      try {
        ...
        if (data.secure_url) {
          const isPortalEquipe = window.location.pathname.includes('equipe.html');
          const inputElId = isPortalEquipe ? 'chatInputEquipe' : (_recordingType === 'Equipe' ? 'chatInputEquipeAdmin' : 'chatInput');
          const inputEl = document.getElementById(inputElId);
          
          if (inputEl) {
            inputEl.value = `[AUDIO]${data.secure_url}`;
            if (isPortalEquipe && window.enviarMsgEquipe) window.enviarMsgEquipe();
            else if (_recordingType === 'Equipe' && window.enviarMsgEquipeAdmin) window.enviarMsgEquipeAdmin();
    ...
    btn.innerHTML = '⏹️';
    window.toastOk && toastOk('Gravando... Clique no ⏹️ vermelho para enviar.');

  } catch (err) {
    window.toastErr && toastErr('⚠ Permissão de microfone negada pelo navegador.');
  }
};
...
window.enviarArquivoChat = async function(input) {
  ...
  const isEquipe = input.id === 'chatFileEquipe' || window.location.pathname.includes('equipe.html');
  const isPortalEquipe = window.location.pathname.includes('equipe.html');
  const inputElId = isPortalEquipe ? 'chatInputEquipe' : (isEquipe ? 'chatInputEquipeAdmin' : 'chatInput');
  const inputEl = document.getElementById(inputElId);
  if (!inputEl) return;
  ...
  try {
    ...
    if (data.secure_url) {
      ...
      if (isPortalEquipe && window.enviarMsgEquipe) window.enviarMsgEquipe();
      else if (isEquipe && window.enviarMsgEquipeAdmin) window.enviarMsgEquipeAdmin();
      else if (window.enviarChat) window.enviarChat();
    }
  } catch(e) {
...
// ============================================================
// CHAT CRM B2C E CHAT EQUIPE ADMIN (Lógica de Tela)
// ============================================================
